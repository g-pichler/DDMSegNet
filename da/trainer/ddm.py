...
class DDMTrainer(gpkeras.trainer.KerasTrainer):
    ...
    def __init__(self, args):
        ...
        self.register(self.setup_callback_evaluator, gpkeras.trainer.Stages.callbacks)

    def setup_callback_evaluator(self):
        ...
        class MyEvaluator:
            def __init__(self,
                ...
                for name, cls in zip(('SRC', 'TARG'), (args.source_class, args.target_class)):
                    for test_labeled_item in dataset.test_labeled_list:
            ...
            def __call__(self, epoch, logs):
                if epoch != 0 and (epoch + 1) % self._interval != 0:
                    ...
                    for k in self.evaluators.keys():
                        for key in ["{k}_DC_{i}".format(k=k, i=i) for i in range(self.n_classes)]:
                            if key in logs:
                                del logs[key]
                        if "{k}_DC".format(k=k) in logs:
                            del logs["{k}_DC".format(k=k)]
                    return
                for k, eval_list in self.evaluators.items():
                    ...
                    for i, dc in enumerate(dice_coeff):
                        logs["{k}_DC_{i}".format(k=k, i=i)] = dc
                        if i > 0:
                            # zero is background
                            mean_dc += dc
                    mean_dc /= len(dice_coeff) - 1
                    logs["{k}_DC".format(k=k)] = mean_dc
                    dice_str = "[" + " ".join(["{:.3f}".format(x) for x in dice_coeff]) + "]"
                    logger.info("DICE_{k:6.6}: {dice_str}, avg: {mean_dc:.3f}".format(k=k,
                                                                                      dice_str=dice_str,
                                                                                      mean_dc=mean_dc))
